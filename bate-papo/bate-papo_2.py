...
credentials = pika.PlainCredentials("chat_2", "chat_2")
...
def receptor():
    ...
    if (canal.basic_consume(queue="fila_1", on_message_callback=chamada, auto_ack=True)):
...
def envio():
    ...
    canal.basic_publish(exchange="", routing_key="fila_2", body=live)
    conexao.close()
